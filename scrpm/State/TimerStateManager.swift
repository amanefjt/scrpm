...
final class TimerStateManager {
    ...
    /// 既存の入力検出（idleActivationMinutes、設定画面で変更可能）が自動開始してくれる。
    ///
    ...
    // Idle 中: 「作業をやめる」後に PC 作業を続けている場合、idleActivationMinutes 分で
    // 無音で自動開始する。
    // Working 中: 逆に workInactivityDeactivationMinutes 分操作がなければ、手を止めている
    // （＝作業していない）とみなし無音で自動的に「作業をやめる」扱いにする
    // （中断セッションとして記録）。
    // どちらも既定値は5分で、設定画面（SettingsView）から変更できる。
